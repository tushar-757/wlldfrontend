...
export const registerAdminHandler = async (FirstName, LastName, Email, PhoneNo, Password, dispatch) => {
  try {
...
export const loginAdminHandler = async (e, email, password, navigate) => {
  ...
  try {
    ...
    localStorage.setItem('admin_id', registerUser?.data?.result?.adminPresent?._id);
...
export const updateAdminHandler = async (FirstName, LastName, Email, PhoneNo) => {
  try {
...
export const changePasswordHandler = async (adminName, OldPassword, NewPassword, ConfirmPassword) => {
  try {
